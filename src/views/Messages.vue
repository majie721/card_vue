        ...
        <div>
            <van-collapse v-model="activeNames" @change="readMessg">
                ...
                <van-collapse-item v-for="item in list" :key="item.id"  :name="item.id" >
                    <div slot="title" ><span class="tab-title" >{{item.title}}<van-icon v-if="item.status!=3" name="chat-o" dot /></span><span class="tab-time">{{item.created_at}}</span></div>
                    <div>
                        {{item.content}}
                    </div>
                </van-collapse-item>
                ...
                for(let key in this.list){
                    if(this.list[key]['id']==id){
                        if(this.list[key]['status']!=3){
                            ...
                            this.$api.read_message(param).then(
                                res=>{
                                    console.log(res);
                                }
                            )
