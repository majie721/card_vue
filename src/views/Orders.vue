      ...
      >
        ...
        >
          <div slot="title" class="otitle">
            <van-row>
              ...
              <van-col span="6" style="text-align: end">
                <span class="ostatus" :style="StatusColor(item.status)">{{
                  item.status_name
                }}</span>
              </van-col>
            </van-row>
          ...
          <div slot="tags" style="margin-top: 3px">
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;
            <van-tag v-if="item.urgent > 0" type="danger"
              >加急处理{{ item.urgent }}</van-tag
            >
          </div>
          <div slot="desc">
            <van-row>
              <van-col span="10"> 订单ID: &nbsp;&nbsp;{{ item.id }} </van-col>
            ...
            <van-row>
              <van-col span="20">
                ...
                <span style="display: inline-block;margin-left: 10px"
                  ><span style="font-size: 14px">{{ item.faceval }}</span
                  >元</span
                >
              </van-col>
              <van-col span="4">
            ...
            <van-row class="amount_row">
              <van-col span="8" @click="$toast('提交总额 = 提交的张数×面值')">
                <div class="th_amount ath_left">
                  提交总额<van-icon name="question-o" />
                </div>
                <div class="td_amount atd_left">{{ item.total_amount }}</div>
              </van-col>
              <van-col
                span="8"
                @click="$toast('成功总额 = 处理成功的张数×面值')"
              >
                <div class="th_amount ath_center">
                  成功总额 <van-icon name="question-o" />
                </div>
                <div class="td_amount atd_center">
                  {{ item.success_amount }}
                </div>
              </van-col>
              <van-col
                span="8"
                @click="$toast('结算总额 = 成功总额×️折扣-加急处理费')"
              >
                <div class="th_amount ath_right">
                  结算总额<van-icon name="question-o" />
                </div>
          ...
          <div slot="footer">
            <van-button
              ...
              >复制订单号</van-button
            >
            <van-button type="info" @click="toDetails(item.id)" size="small"
              >订单详情</van-button
            >
          </div>
        </van-card>
...
export default {
  ...
  methods: {
    ...
    tabChange(name, title) {
      this.list = [];
      this.loading = true;
      (this.finished = false), (this.page = 1), this.onLoad();
    },
    copyOrderID(id) {
