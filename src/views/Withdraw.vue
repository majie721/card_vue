<template>
  <div>
    <div>
      <van-nav-bar title="提现" @click-left="$router.go(-1)" left-arrow>
    ...
    <div class="money">
      <img
        ...
        src="https://hm-1258945893.cos.ap-chengdu.myqcloud.com/card/money1.png"
        alt=""
      />
    </div>
    <div>
    ...
    <div style="text-align: center">
      <div>
        <van-button :disabled="money <= 0 || money=='--.--'" type="primary" @click="withdrawTip()"
          >立即提现</van-button
        >
      </div>
      <div style="margin-top: 20px">
        <van-button type="info" @click="$toast('邮件功能正在开发中')">发送账单</van-button>
      </div>
    </div>

    <van-dialog v-model="isShow" show-cancel-button title="提现信息">
      <p class="confrim-p">
        <span class="confrim-lable">账号类型:</span> {{ currentAccount.name }}
      </p>
      <p class="confrim-p">
        <span class="confrim-lable">收款账号:</span>
      ...
      <p class="confrim-p">
        <span class="confrim-lable">金额:</span> {{ this.money }}
      </p>
    </van-dialog>
          ...
          >
            ...
            >
              ...
              <van-col span="4"
                ><img
                  class="account-image"
                  @click="currentAccount = item"
                  :src="typePng(item.type)"
                  alt=""
                />
              </van-col>
              <van-col span="14" @click="currentAccount = item">
                ...
                <div style="padding-left: 10px;color:gray;font-size: small">
                  {{ item.account_name }}
                </div>
              </van-col>
            </van-row>
  ...
  methods: {
    ...
    withdraw: function() {
      this.$api
        ...
        .catch(this.$toast.fail("提现失败,请联系管理员处理"));
    },
